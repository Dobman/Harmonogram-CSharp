...
namespace Harmonogram
{
    class DayOffWork
    {
        public static bool IsDayOff(DateTime date)
        {
            ...
            if (date.Month == 01 && date.Day == 01 && (date.Year >= 1952 && date.Year <= 1960)) return true; // Trzech Króli 
            if (date.Month == 05 && date.Day == 01) return true; // 1 maja 
            if (date.Month == 05 && date.Day == 03 && (date.Year >= 1918 && date.Year <= 1950 || date.Year >= 1990)) return true; // 3 maja
